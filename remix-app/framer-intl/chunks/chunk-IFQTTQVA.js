...
// /:https://framerusercontent.com/modules/wFkXxZqfKOPeEQtsYrsa/Xcw7qvncyogRPQtf9aNn/codemirror_state.js
var Text = class {
  /**
...
function flatten(extension, compartments, newCompartments) {
  ...
  function inner(ext, prec2) {
    ...
    if (Array.isArray(ext)) {
      ...
      if (!content1) throw new Error(`Unrecognized extension value in extension set (${ext}). This sometimes happens because multiple instances of @codemirror/state are loaded, breaking instanceof checks.`);
...
var ContentView = class {
  ...
  sync(view, track) {
    if (this.dirty & 2) {
      ...
      for (let child1 of this.children) if (child1.dirty) {
...
function nearbyTextNode(startNode, startOffset, side) {
  ...
  if (side >= 0) for (let node1 = startNode, offset1 = startOffset; ; ) {
    ...
    if (change.from >= sel.from && change.to <= sel.to && change.to - change.from >= (sel.to - sel.from) / 3 && (!newSel || newSel.main.empty && newSel.main.from == change.from + change.insert.length) && view.inputState.composing < 0) {
      ...
      if (startState.selection.ranges.length > 1 && view.inputState.composing >= 0 && change.to <= sel.to && change.to >= sel.to - 10) {
        ...
        tr = startState.changeByRange((range) => {
          ...
          if (range.to - range.from != size || view.state.sliceDoc(from, to) != replaced || // changes in the same node work without aborting
...
// /:https://framerusercontent.com/modules/CO9ruPlVDRTCLVInyri7/dIK6ma4pzS0qSHWtJ4s1/lezer_common.js
...
function resolveNode(node, pos, side, overlays) {
  ...
  if (overlays) for (let scan = node, parent1 = scan.parent; parent1; scan = parent1, parent1 = scan.parent) {
    ...
    for (let parent = this; ; ) {
      for (let { children, positions } = parent._tree, e = dir > 0 ? children.length : -1; i2 != e; i2 += dir) {
        ...
        if (next instanceof TreeBuffer) {
          ...
          let mounted;
          if (!(mode & IterMode.IgnoreMounts) && next.props && (mounted = next.prop(NodeProp.mounted)) && !mounted.overlay) return new TreeNode(mounted.tree, start, i2, parent);
...
var TreeCursor = class {
  ...
  atLastNode(dir) {
    ...
    for (; parent; { index, _parent: parent } = parent) {
      if (index > -1) for (let i1 = index + dir, e = dir < 0 ? -1 : parent._tree.children.length; i1 != e; i1 += dir) {
    ...
    scan: for (let nest, isCovered; this.stoppedAt == null || cursor.from < this.stoppedAt; ) {
      ...
      if (fragmentCursor.hasNode(cursor)) {
        ...
        if (typeof nest.overlay == "function") {
          ...
          if (ranges.length) this.inner.push(new InnerParse(nest.parser, nest.parser.startParse(this.input, enterFragments(oldMounts, ranges), ranges), nest.overlay ? nest.overlay.map((r) => new Range2(r.from - cursor.from, r.to - cursor.from)) : null, cursor.tree, ranges));
      ...
      if (enter && cursor.firstChild()) {
        ...
        for (; ; ) {
          ...
          if (overlay && !--overlay.depth) {
            ...
            if (ranges1.length) this.inner.splice(overlay.index, 0, new InnerParse(overlay.parser, overlay.parser.startParse(this.input, enterFragments(overlay.mounts, ranges1), ranges1), overlay.ranges.map((r) => new Range2(r.from - overlay.start, r.to - overlay.start)), overlay.target, ranges1));
...
// /:https://framerusercontent.com/modules/rOWwbZHN39cczduPnzmw/OTf12FftmvJsuTNsZb3G/lezer_highlight.js
var nextTagID = 0;
var Tag = class {
  /**
...
var LanguageDescription = class {
  ...
  */
  static matchLanguageName(descs, name2, fuzzy = true) {
    ...
    if (fuzzy) for (let d1 of descs) for (let a of d1.alias) {
    ...
    if (dir < 0 ? cursor.to <= token.from : cursor.from >= token.to) {
      if (depth == 0 && matching.indexOf(cursor.type.name) > -1 && cursor.from < cursor.to) {
        ...
        if (depth == 0) {
          ...
          return { start: firstToken, end: endHandle1 && endHandle1.from < endHandle1.to ? { from: endHandle1.from, to: endHandle1.to } : void 0, matched: false };
        }
        depth--;
      }
    }
...
// /:https://framerusercontent.com/modules/MiFWyNBz6FaRwOOEYJ4H/RMT2Q4ScDAUq3xYhahWs/lezer_lr.js
var Stack = class {
  /// @internal
  toString() {
  ...
  recoverByInsert(next) {
    ...
    if (nextStates.length > 4 << 1 || this.stack.length >= 120) {
      ...
      if (this.stack.length < 120) for (let i1 = 0; best.length < 4 << 1 && i1 < nextStates.length; i1 += 2) {
...
var Parse2 = class {
  ...
  advance() {
    ...
    if (this.recovering && stopped) {
      let finished1 = this.stoppedAt != null && stopped[0].pos > this.stoppedAt ? stopped[0] : this.runRecovery(stopped, stoppedTokens, newStacks);
...
var LRParser = class extends Parser {
  ...
  parseDialect(dialect) {
    ...
    for (let i2 = 0; i2 < values.length; i2++) if (!flags[i2]) {
      for (let j = this.dialects[values[i2]], id21; (id21 = this.data[j++]) != 65535; ) (disabled || (disabled = new Uint8Array(this.maxTerm + 1)))[id21] = 1;
    }
