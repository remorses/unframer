...
// /:https://framerusercontent.com/modules/ucIqmdXFlkfhojY0CsdL/fH5TW8VYLZR7eKa8JOHL/lezer_markdown.js
    ...
    if (nonEmpty(config.parseInline)) {
      for (let spec21 of config.parseInline) {
        ...
        if (found1 > -1) {
          ...
          let pos1 = spec21.before ? findName(inlineNames, spec21.before) : spec21.after ? findName(inlineNames, spec21.after) + 1 : inlineNames.length - 1;
...
// /:https://framerusercontent.com/modules/ew2kgXFUVzaNY5ZWXtC5/G0go5tjEUs1iWpGXELvD/codemirror_lang_markdown.js
  ...
  let dont = null, changes = state.changeByRange((range) => {
    ...
    if (inner.node.name == "Blockquote" && emptyLine && line.from) {
      ...
      if (quoted && quoted.index == inner.from) {
        let changes31 = state.changes([{ from: prevLine.from + quoted.index, to: prevLine.to }, { from: line.from + inner.from, to: line.to }]);
    ...
    if (!continued || /^[\s\d.)\-+*>]*/.exec(line.text)[0].length >= inner.to) {
      for (let i1 = 0, e1 = context.length - 1; i1 <= e1; i1++) {
        insert += i1 == e1 && !continued ? context[i1].marker(doc, 1) : context[i1].blank(i1 < e1 ? context[i1 + 1].from - insert.length : null);
      }
    }
    let from = pos;
  ...
  let dont = null, changes = state.changeByRange((range) => {
    ...
    if (range.empty && markdownLanguage.isActiveAt(state, range.from)) {
      ...
      if (context.length) {
        ...
        if (pos - line.from == spaceEnd && // construct's syntax, or there's only indentation in the
        // target range
...
var __FramerMetadata__ = { "exports": { "markdownLanguage": { "type": "variable", "annotations": { "framerContractVersion": "1" } }, "markdown": { "type": "function", "annotations": { "framerContractVersion": "1" } }, "insertNewlineContinueMarkup": { "type": "variable", "annotations": { "framerContractVersion": "1" } }, "deleteMarkupBackward": { "type": "variable", "annotations": { "framerContractVersion": "1" } }, "markdownKeymap": { "type": "variable", "annotations": { "framerContractVersion": "1" } }, "commonmarkLanguage": { "type": "variable", "annotations": { "framerContractVersion": "1" } }, "__FramerMetadata__": { "type": "variable" } } };
export {
  __FramerMetadata__,
  commonmarkLanguage,
