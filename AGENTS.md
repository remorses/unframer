...
1. Add a patch to `fixFramerCode()` in `scripts/download.ts`
...
Always use the `replaceOrThrow` helper, **never a bare `.replace()`**. Framer ships a new runtime bundle every day and regularly renames the code we patch. A `.replace()` that stops matching is a silent no-op: the workflow keeps passing and ships a subtly broken `framer.js`.
...
## When the daily Framer download workflow fails

Read [docs/framer-download-workflow.md](./docs/framer-download-workflow.md) before doing anything.

It covers the triage step that tells an expired `FRAMER_SESSION` cookie apart from a Framer runtime refactor (they look nothing alike but get confused constantly), how to refresh the cookie with playwriter and update the GitHub + Doppler secrets, and how to fix a thrown `replaceOrThrow` patch.

Quick triage: if the failed run log reaches `framer.js size is X Mb`, authentication worked and the cookie is **not** the problem.
